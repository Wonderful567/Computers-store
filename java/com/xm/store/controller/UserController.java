...
import com.xm.store.controller.ex.*;
...
public class UserController extends BaseController {
    ...
    public JsonResult<User> login(String username, String password, HttpSession session) {
        ...
        User data = userServiceImpl.login(username, password);
    ...
    public JsonResult<Void> changePassword(String oldPassword, String newPassword, HttpSession session) {
        ...
        // 调用业务对象执行修改密码
        userServiceImpl.changePassword(uid, username, oldPassword, newPassword);
        // 返回成功
        return new JsonResult<Void>(OK);
    }

    //前端获取用户信息GetMapping("get_by_uid")
    @GetMapping("get_by_uid")
    public JsonResult<User> getByUid(HttpSession session) {
        // 从HttpSession对象中获取uid
        Integer uid = getUidFromSession(session);
        // 调用业务对象执行获取数据
        User data = userServiceImpl.getByUid(uid);
        // 响应成功和数据
        return new JsonResult<User>(OK, data);
    }
    ...
    public JsonResult<Void> changeInfo(User user, HttpSession session) {
        ...
        String username = getUsernameFromSession(session);
        // 调用业务对象执行修改用户资料
        userServiceImpl.changeInfo(uid, username, user);
        // 响应成功
        return new JsonResult<Void>(OK);
    }
    ...
    public JsonResult<String> changeAvatar(@RequestParam("file") MultipartFile file, HttpSession session) {
        ...
        String username = getUsernameFromSession(session);
        // 将头像写入到数据库中
        userServiceImpl.changeAvatar(uid, username, avatar);
